...
void test_all(uint16_t ref, const uint8_t *buffer, size_t size, uint64_t initial) {
    ...
    if (fastcsum_avx2_usable()) {
        ...
        TEST_CSUM(ref, fastcsum_nofold_avx2_v8, buffer, size, initial);
    }
#endif
...
TEST_CASE("bench") {
    ...
    if (fastcsum_avx2_usable()) {
        ...
        BENCHMARK("avx2_v8") {
            return fold_complement_checksum64(fastcsum_nofold_avx2_v8(pkt.data(), pkt.size(), 0));
        };
    }
#endif
    BENCHMARK("vec256") {
...
TEST_CASE("bench-unaligned") {
    ...
    if (fastcsum_avx2_usable()) {
        ...
        BENCHMARK("avx2_v8") {
            return fold_complement_checksum64(fastcsum_nofold_avx2_v8(pkt.get() + off, size - off, 0));
        };
    }
#endif
    BENCHMARK("vec256") {
