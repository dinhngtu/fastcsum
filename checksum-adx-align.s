...
.global checksum_raw_nofold_adx_align

checksum_raw_nofold_adx_align:
    # rdi: buffer byte ptr
    # rsi: size
    # rdx: initial
    ...
    mov r10, 1
    shl r10, cl
    dec r10                         # r10 is now byte mask according to edx
    and r8, r10

    # in case we're on an odd-byte alignment offset
    # e.g. 00 00 00 00 00 00 00 [01 | 02 03 04 05 06 07 08] 00 | 00 00 ...
    # we must pad with 1 zero byte then byteswap the result:
    # ac = htons(fold(0x0100+0x05040302+0x0706+0x08))
    test edx, 1
    jz 0f
    shl r8, 8
